        ...
        stage('DeployToStaging') {
            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'webserver_login', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        sh "echo $USERNAME"
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$stg_ip \"docker pull kdevesh/train-schedule:${env.BUILD_NUMBER}\""
                        try {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$stg_ip \"docker run --restart always --name train-schedule -p 8080:8080 -d kdevesh/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
